...
class TransmittanceCH4Interpolation:
    def __init__(
        ...
        self.clip_values_retrieval = clip_values_retrieval
        self.allow_negative_ch4 = allow_negative_ch4

        self.cache: Dict[str, Callable[[NDArray], NDArray]] = {}

    def load_interpolation_functions_satellite(
        self, satellite: str
    ) -> Dict[str, Callable[[NDArray], NDArray]]:
    ...
    def _load_interpolation_functions_satellite(
        self, satellite: str
    ) -> Dict[str, Callable[[NDArray], NDArray]]:
    ...
    def _transmittance_B12_interpfun(
        ...
        interpfun_B12 = self.load_interpolation_functions_satellite(satellite)[
            "interpolation_funtion_amf_transmittance_b12"
        ]
        transmittance_b12_arr_1d = interpfun_B12(amf)
        mr_ch4_arr_1d = self.f_ch4_mr(amf)
        return interpolate.interp1d(
            mr_ch4_arr_1d,
            transmittance_b12_arr_1d,
            axis=0,
            fill_value="extrapolate",
            kind="cubic",
        )

    def _transmittance_B11_interpfun(
        self, satellite: str, amf: float
    ) -> Callable[[NDArray], NDArray]:
        interpfun_B11 = self.load_interpolation_functions_satellite(satellite)[
            "interpolation_funtion_amf_transmittance_b11"
        ]
        interpfun_B12 = self.load_interpolation_functions_satellite(satellite)[
            "interpolation_funtion_amf_transmittance_b12"
        ]
    ...
    def _transmittance_B12_bg(self, satellite: str, amf: float) -> NDArray:
        interpfun_B12_bg = self.load_interpolation_functions_satellite(satellite)[
            "interpolation_funtion_amf_transmittance_b12_bg"
        ]
        return interpfun_B12_bg(amf)

    def _transmittance_B11_bg(self, satellite: str, amf: float) -> NDArray:
        interpfun_B11_bg = self.load_interpolation_functions_satellite(satellite)[
            "interpolation_funtion_amf_transmittance_b11_bg"
        ]
        return interpfun_B11_bg(amf)

    def transmittance_B12_B11(
        self,
        satellite: str,
        sza: Optional[float],
        vza: Optional[float],
        deltach4: Union[NDArray, GeoTensor],
        amf: Optional[float] = None,
    ) -> Tuple[Union[NDArray, GeoTensor], Union[NDArray, GeoTensor]]:
        r"""
        ...
        Args:
            ...
            sza (Optional[float]): Solar zenith angle in degrees. This is used if `amf` is not provided.
            vza (Optional[float]): View zenith angle in degrees. This is used if `amf` is not provided.
            deltach4 (Union[NDArray,GeoTensor]): Image with $\Delta$XCH$_4$: the methane concentration in ppb.
            amf (Optional[float], optional): Air mass factor. Defaults to None. If not provided, it will be calculated from `sza` and `vza`.
        ...
        transmittance_b12 = mixing_ratio_methane.apply_interpfun_to_image(
            interpfun_B12, ch4, fill_value_default=1
        )
        transmittance_b11 = mixing_ratio_methane.apply_interpfun_to_image(
            interpfun_B11, transmittance_b12, fill_value_default=1
        )
    ...
    def _deltach4_to_ch4(self, deltach4: Union[NDArray, GeoTensor]) -> Union[NDArray, GeoTensor]:
        ...
        Args:
            deltach4 (Union[NDArray, GeoTensor]): Image with $X$CH$_4$: the methane concentration in ppb.
    ...
    def deltach4_from_ratio_transmittance(
        self,
        satellite: str,
        sza: float,
        vza: float,
        ratio_il: Union[NDArray, GeoTensor],
        clip_values_retrieval: Optional[bool] = None,
        allow_negative_ch4: Optional[bool] = None,
        ...
        Args:
            ...
            clip_values_retrieval (Optional[bool], optional): Whether to clip the retrieved methane concentration to be non-negative
                and to clip the input ratio values to [0.3, 1.08].
                Defaults to None, which uses the object attribute `clip_values_retrieval`.
        Returns:
            Union[NDArray, GeoTensor]: Image with $\Delta$XCH$_4$: the methane concentration in ppb.
        """
        amf = self.air_mass_factor(sza, vza)
        mr_ch4_arr_1d = self.f_ch4_mr(amf)

        if clip_values_retrieval is None:
            clip_values_retrieval = self.clip_values_retrieval

        if allow_negative_ch4 is None:
            allow_negative_ch4 = self.allow_negative_ch4
        ...
        #                                  np.clip(mr_ch4_arr_1d - self.background_concentration, 0, None),
        #                                  axis=0,
    ...
    def deltach4_from_B12_transmittance(
        ...
        sza: Optional[float],
        vza: Optional[float],
        trans_b12: Union[NDArray, GeoTensor],
        amf: Optional[float] = None,
    ) -> Union[NDArray, GeoTensor]:
        r"""
        Returns the methane concentration in ppb from the observed transmittance of band B12.

        Args:
            satellite (str): name of the satellite. Either 'S2A', 'S2B', 'LC08' or 'LC09'.
            sza (float): solar zenith angle in degrees. This is used if `amf` is not provided.
            vza (float): view zenith angle in degrees. This is used if `amf` is not provided.
            trans_b12 (Union[NDArray, GeoTensor]): Observed transmittance of band B12.
            amf (Optional[float], optional): Air mass factor. Defaults to None. If not provided, it will be calculated from `sza` and `vza`.
        ...
        interpfun_B12 = self._transmittance_B12_interpfun(satellite, amf)
        all_transmittances_b12 = interpfun_B12(mr_ch4_arr_1d)

        transmittance_b12_bg = self._transmittance_B12_bg(satellite, amf)

        toa_b12_corrected = trans_b12 * transmittance_b12_bg

        interpfun = interpolate.interp1d(
            all_transmittances_b12,
            mr_ch4_arr_1d,
            axis=0,
            fill_value="extrapolate",
            kind="cubic",
        ...
        return mixing_ratio_methane.apply_interpfun_to_image(final_fun, toa_b12_corrected)

    def deltach4_from_B11_transmittance(
        self,
        satellite: str,
        sza: Optional[float],
        vza: Optional[float],
        trans_b11: Union[NDArray, GeoTensor],
        amf: Optional[float] = None,
    ) -> Union[NDArray, GeoTensor]:
        r"""
        Returns the methane concentration in ppb from the observed transmittance of band B11.

        Args:
            satellite (str): name of the satellite. Either 'S2A', 'S2B', 'LC08' or 'LC09'.
            sza (Optional[float]): solar zenith angle in degrees. This is used if `amf` is not provided.
            vza (Optional[float]): view zenith angle in degrees. This is used if `amf` is not provided.
            trans_b11 (Union[NDArray, GeoTensor]): Observed transmittance of band B11.
            amf (Optional[float], optional): Air mass factor. Defaults to None.
                If not provided, it will be calculated from `sza` and `vza`.
        ...
        interpfun_B11 = self._transmittance_B11_interpfun(satellite, amf)
        all_transmittances_b11 = interpfun_B11(mr_ch4_arr_1d)

        transmittance_b11_bg = self._transmittance_B11_bg(satellite, amf)

        toa_b11_corrected = trans_b11 * transmittance_b11_bg

        interpfun = interpolate.interp1d(
            all_transmittances_b11,
            mr_ch4_arr_1d,
            axis=0,
            fill_value="extrapolate",
            kind="cubic",
        ...
        return mixing_ratio_methane.apply_interpfun_to_image(final_fun, toa_b11_corrected)
...
class TransmittanceCH4InterpolationFromLUT(TransmittanceCH4Interpolation):
    def __init__(
        ...
        if self.trans_tot_as_tbg:
            if not self.with_ltoa_correction:
                raise ValueError(
                    "The total atmospheric transmittance is only used when the LTOA correction is enabled.\n Set `trans_tot_as_tbg=False` or `with_ltoa_correction=True`."
                )
    ...
    def t_background_transmittance(self) -> NDArray:
        ...
        self._t_background_transmittance = np.zeros(
            (len(self.amf_arr), len(self.wvl_mod)), dtype=self.t_full_arr.dtype
        )
        for i, amf in enumerate(self.amf_arr):
            transmittance_fun_for_cch4 = self._transmittance_fun_for_cch4(amf)
            self._t_background_transmittance[i] = transmittance_fun_for_cch4(
    ...
    def _load_interpolation_functions_satellite(
        ...
        self.b12srf_interpfunc, self.b11srf_interpfunc = mixing_ratio_methane.load_srfinterpfun(
            satellite
        )
        ...
        if self.with_ltoa_correction:
            # \int_{B12} E_g(\lambda) T_{atm}(\lambda) T_{Bg\text{CH}_4 + \Delta \text{XCH}_4}(\lambda) d\lambda
            if self.trans_tot_as_tbg:
                transmittance_b12 = (
                    self.eg_arr
            ...
            transmittance_b12 = (self.t_full_arr * b12srf_interp).sum(
                axis=-1
        ...
        transmittance_b11 = (self.t_full_arr * b11srf_interp).sum(
            axis=-1
        ...
        interpolation_funtion_amf_transmittance_b12 = interpolate.interp1d(
            self.amf_arr, transmittance_b12, axis=0, kind="cubic"
        )
        interpolation_funtion_amf_transmittance_b11 = interpolate.interp1d(
            self.amf_arr, transmittance_b11, axis=0, kind="cubic"
        )
        interpolation_funtion_amf_transmittance_b12_bg = interpolate.interp1d(
            self.amf_arr, transmittance_b12_bg, axis=0, kind="cubic"
        )
        interpolation_funtion_amf_transmittance_b11_bg = interpolate.interp1d(
            self.amf_arr, transmittance_b11_bg, axis=0, kind="cubic"
        )
...
def export_transmittances(
    ...
    if satellites is None:
        satellites = ["S2A", "S2B", "S2C", "LC08", "LC09", "LE07", "LT04", "LT05"]
    out: Dict[str, Any] = {}

    simulator = TransmittanceCH4InterpolationFromLUT(
        lut_file=lut_file,
