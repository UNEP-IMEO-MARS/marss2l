...
"""
BACKGROUND_CONCENTRATION = 1_800  # ppb
MAX_CH4_CONCENTRATION_LUT = 16_000  # ppb

ATMOSPHERE_HEIGHT_METHANE = 8_000  # m
SIGMA_CH4_S2_PPB = 205.64270005117675  # ppb
...
def convert_units(
    data: Union[NDArray, GeoTensor, float, int], units_src: str, units_dst: str
    ...
    if units_src != "ppm x m":
        if units_src == "ppm":
            ...
            raise ValueError(
                f"units_src must be one of 'ppm', 'ppb', or 'ppm x m' found {units_src}"
            )
...
def obtain_flux_rate(
    ...
    a_u_eff: float = A_UEFF_S2,
    b_u_eff: float = B_UEFF_S2,
    a_std: float = 0.01,
    b_std: float = 0.01,
    ...
    assert (
        methane_enhancement_image.shape == plume_mask_binary.shape
    ...
    assert (
        len(methane_enhancement_image.shape) == 2
    ...
    methane_enhancement_image_values = convert_units(
        methane_enhancement_image_values, "ppb", "ppm x m"
    )
    ...
    ime = (
        np.sum(methane_enhancement_image_values[binary_mask])
        * np.prod(resolution)
        * 1_000
        * 0.01604
    ) / (1e6 * 22.4)
    ...
    sig_IME = (sig_xch4 * np.sqrt(npix_plume) * np.prod(resolution) * 1_000 * 0.01604) / (
        1e6 * 22.4
    )
