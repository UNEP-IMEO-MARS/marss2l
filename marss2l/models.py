...
class UnetFiLMRefactor(nn.Module):
    def __init__(
        ...
        self.film_params = nn.ParameterList(
            [
                torch.nn.Parameter(
                    init_weights_film(
                        self.n_locs, 64 // div_factor if self.one_param_per_channel else 1
                    )
                ),
                torch.nn.Parameter(
                    init_weights_film(
                        self.n_locs, 128 // div_factor if self.one_param_per_channel else 1
                    )
                ),
                torch.nn.Parameter(
                    init_weights_film(
                        self.n_locs, 256 // div_factor if self.one_param_per_channel else 1
                    )
                ),
                torch.nn.Parameter(
                    init_weights_film(
                        self.n_locs, 512 // div_factor if self.one_param_per_channel else 1
                    )
                ),
                torch.nn.Parameter(
                    init_weights_film(
    ...
    def forward(self, x):
        ...
        x1 = self.activation(
            self.do_film(x1, film_params[0], inc=True)
        )  # x1*film_params[0]+film_params[9])
        x2 = self.down1(x1)
...
def load_model(
    ...
    if model_name == "film":
        logger.info("FiLM")
        assert max_index_film is not None and (
            max_index_film > 0
        ), f"max_index_film should not be None if model_name is film provided {max_index_film}"

        model = UnetFiLMRefactor(
