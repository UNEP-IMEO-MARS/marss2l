...
def plot_prob_vs_emission_rate(
    df_plot: pd.DataFrame, figsize: tuple[int, int] = (13, 7)
    ...
    data["interval_ch4_fluxrate_str"] = data["interval_ch4_fluxrate_str"].cat.remove_categories(
        "[0, 0]"
    )
    ...
    sns.boxplot(
        ...
        y="scenepredcontinuous",
        hue="model_name",
        ax=ax2,
        palette=PALETTE_ALL[: len(model_names_plot)],
    )
    ax2.legend(loc="lower right")
    sns.countplot(data=df_plot_hist_fluxrate, x="interval_ch4_fluxrate_str", ax=ax4, color=C1)
