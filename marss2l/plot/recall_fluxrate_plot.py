...
from marss2l.metrics import fpr, get_scenelevel_metrics
from marss2l.plot.colors import C0, C1, C2, C3, C4

THRESHOLD_MARSS2L = 0.5
THRESHOLD_MBMP = -0.985
...
for i, fluxrate in enumerate(loaders.INTERVALS_FLUXRATE[:-1]):
    if i == len(loaders.INTERVALS_FLUXRATE) - 2:
        ...
        INTERVALS_FLUXRATE_STR.append(
            f"({fluxrate_to_str(fluxrate)}, {fluxrate_to_str(loaders.INTERVALS_FLUXRATE[i + 1])}]"
        )


def plot_recall_fpr_fluxrate(
    ...
    Args:
        detections_dataframe (pd.DataFrame): Dataframe containing the detections with the following columns:
            ...
            - isplumeprednum: 1 if the model predicts a plume, 0 otherwise
            - ch4_fluxrate_th: flux rate of the plume of the image
            - scenepredcontinuous: continuous prediction score for the scene
            - isplumenum: numeric version of isplume (0/1)
        ...
        y_metric (str): "recall" or "average_score"
    ...
    """
    assert y_metric in {"average_score", "recall"}, f"{y_metric} unknown"
    ...
    for fluxrateidx, flux_rate_thr in enumerate(xticks):
        for model, df_model in detections_dataframe.groupby("model_name"):
            ...
            fluxrate_thr_end = (
                xticks[fluxrateidx + 1]
                if fluxrateidx < len(xticks) - 1
                else loaders.INTERVALS_FLUXRATE[-1]
            )
            ...
            if cummulative:
                ...
                dg = df_model[
                    (df_model.ch4_fluxrate_th > flux_rate_thr)
                    & (df_model.ch4_fluxrate_th <= fluxrate_thr_end)
                ]
            ...
            mets.append(
                {
                    ...
                    "average_score": np.mean(dg.scenepredcontinuous),
    ...
    # Compute FPR for barplot
    mets = []
    for model, dg in detections_dataframe.groupby("model_name"):
        mets_iter = {"fpr": fpr(dg.isplumenum, dg.isplumeprednum)}
        mets_iter.update(
            {
                "nsamples": dg.shape[0],
                "nplumes": dg.isplumenum.sum(),
    ...
    if order_models is None:
        ...
        mets_recall_vs_fluxrate = mets_recall_vs_fluxrate[
            mets_recall_vs_fluxrate.model_name.isin(order_models)
        ].copy()
    ...
    # Plot the FPR
    ax1.bar(
        overall_mets.model_name,
        overall_mets.fpr,
        color=[C0, C4, C2, C3],
        label=overall_mets.model_name,
        alpha=0.9,
    )
    ax1.set_ylabel("FPR")
    ...
    df_plot_hist_fluxrate = mets_recall_vs_fluxrate.loc[
        mets_recall_vs_fluxrate.model_name == order_models[0],
        ["interval_ch4_fluxrate_str", "nplumes_interval"],
    ].copy()
    ...
    for fluxrateinterval in INTERVALS_FLUXRATE_STR[1:]:
        if fluxrateinterval not in df_plot_hist_fluxrate.interval_ch4_fluxrate_str.values:
            df_plot_bar.append(
                {"interval_ch4_fluxrate_str": fluxrateinterval, "nplumes_interval": 0}
            )
        else:
            df_plot_bar.append(
                df_plot_hist_fluxrate[
                    df_plot_hist_fluxrate.interval_ch4_fluxrate_str == fluxrateinterval
                ]
                .iloc[0]
                .to_dict()
            )
    ...
    axtwin.bar(
        df_plot_bar.interval_ch4_fluxrate_str,
    ...
    mets_show = mets_recall_vs_fluxrate[
        ["interval_ch4_fluxrate_str", "recall", "average_score", "model_name"]
    ].copy()
    sns.lineplot(
        mets_show,
        x="interval_ch4_fluxrate_str",
        y=y_metric,
        hue="model_name",
        ax=ax,
        style="model_name",
    ...
    ax.set_yticks(yticks_recall, labels=[f"{int(i*100)}%" for i in yticks_recall])
    ...
    if cummulative:
        ax.set_ylabel(f"Cumulative {y_metric.replace('_', ' ')}")
    else:
        ax.set_ylabel(f"{y_metric.replace('_', ' ').capitalize()}")
