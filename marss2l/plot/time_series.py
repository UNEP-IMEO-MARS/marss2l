...
def plot_time_series(
...
) -> Union[plt.Axes, Tuple[plt.Figure, list[plt.Axes]]]:
    """
    ...
    mlis_df.loc[
        mlis_df.isplumestr == "cloudy/bad retrieval", ["ch4_fluxrate", "ch4_fluxrate_std"]
    ] = pd.NA
    ...
    if cluster_sentinel2_and_landsat:
        ...
        mlis_df.satellite = mlis_df.satellite.apply(
            lambda x: "L8/9" if x.startswith("LC08") or x.startswith("LC09") else x
        )

    if colors_satellite is None:
        colors_satellite = {
            satame: color for satame, color in zip(mlis_df.satellite.unique(), plt.cm.tab10.colors)
        }
    ...
    def plot_subset(df, ax: plt.Axes, plot_legend: bool = False):
        ...
        for i, (satame, data) in enumerate(df_positive.groupby("satellite")):
            ...
            ax.scatter(
                data_neg.tile_date, data_neg.ch4_fluxrate.values, color=colors_satellite[satame]
            )
        if plot_legend:
    ...
    fig, axs = plt.subplots(nsplits, 1, figsize=(width, nsplits * height), tight_layout=True)
