...
def plot_location(
    ...
    if start_date is not None:
        ...
        bool_date = (dataset.image_dataframe.tile_date >= start_date) & (
            dataset.image_dataframe.location_name == location_name
        )
        if not bool_date.any():
            raise ValueError(f"Start date {start_date} not found in the dataset")

        # Items to plot
        items_plot = dataset.image_dataframe.loc[bool_date].sort_values("tile_date")

        indexes_plot = items_plot.index.tolist()
    else:
        items_plot = dataset.image_dataframe.loc[
            dataset.image_dataframe.location_name == location_name
        ].sort_values("tile_date")
        if start_ind >= len(items_plot):
            raise ValueError(
                f"Start index {start_ind} out of range. Max index is {len(items_plot)-1}"
            )
    ...
    for count in range(n_panels):
        ...
        item = dataset[index]
        date = dataset.image_dataframe.loc[index, "tile_date"]
        satellite = dataset.image_dataframe.loc[index, "satellite"]
        ...
        axs[idxplot][count].imshow(
            item["y_target"], vmin=0, vmax=1, cmap="magma", interpolation="nearest"
        )
        idxplot += 1
