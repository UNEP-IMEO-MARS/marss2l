...
from .colors import C0, C1, C2, C4, PALETTE_ALL


def calculate_images_by_yearmonth_and_summary(
    dataframe_data_traintest: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    images_by_yearmonth_total = (
        dataframe_data_traintest.groupby(["year_month"])["isplume"]
        .agg(["count", "sum"])
        .rename({"count": "# images", "sum": "# plumes"}, axis=1)
    )
    images_by_yearmonth_total["# locs"] = dataframe_data_traintest.groupby(["year_month"])[
        "location_name"
    ].nunique()
    images_by_yearmonth_total = images_by_yearmonth_total.reset_index()

    summaries = (
        dataframe_data_traintest.groupby("split_name")["isplume"]
        .agg(["sum", "count"])
        .rename(columns={"sum": "nplumes", "count": "nimages"})
    )
...
def _plot_images_by_month(
    images_country_by_yearmonth,
    ax: plt.Axes,
    ...
    lty = ax.plot(
        images_country_by_yearmonth["year_month"],
    ...
    if datetime_end is None:
        datetime_end = images_country_by_yearmonth.year_month.max() + timedelta(
            days=step_in_months * 30
        )
    ...
    lty2 = axtwin.plot(
        images_country_by_yearmonth["year_month"],
        images_country_by_yearmonth["# plumes"],
        c=C2,
        label="# plumes",
    )
    lty3 = axtwin.plot(
        images_country_by_yearmonth["year_month"],
        images_country_by_yearmonth["# locs"],
        c=C4,
        label="# locs",
    )
    ...
    t = np.arange(
        datetime_start, datetime_end, np.timedelta64(step_in_months, "M"), dtype="datetime64[M]"
    )
    ...
    if plot_legend:
        legend1 = plt.legend(lty + lty3 + lty2, ["# images", "# locs", "# plumes"], loc=loc_legend)
...
def add_date_region(
    ...
    if stats is not None:
        ...
        # Add background box
        rect = Rectangle(
            (mid_point_num - box_width_data / 2, box_y),
            width=box_width_data,
            height=box_height,
            transform=ax.get_xaxis_transform(),
            facecolor="white",
            edgecolor="gray",
...
def plot_images_by_month(
    ...
    images_country_by_yearmonth, summaries = calculate_images_by_yearmonth_and_summary(
    ...
    add_date_region(
        ...
        stats=stats,
        plot_start_line=False,
    )

    stats = summaries.loc["val"].to_dict() if "val" in summaries.index else None
    add_date_region(ax, datetime(2021, 1, 1), datetime(2021, 12, 31), "Val", stats=stats)

    add_date_region(ax, datetime(2022, 1, 1), datetime(2023, 11, 30), "Train", stats=None)
